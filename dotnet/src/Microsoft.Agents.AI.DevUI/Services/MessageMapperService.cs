...
/// </summary>
internal sealed class MessageMapperService
{
    ...
    private sealed class ConversionContext
    {
        ...
        public Dictionary<string, FunctionCallInfo> ActiveFunctionCalls { get; set; } = [];
    }
    ...
    {
        return this._contexts.GetOrAdd(sessionId, _ => new ConversionContext());
    }
    ...
    public async Task<IEnumerable<object>> ConvertEventAsync(object rawEvent, DevUIExecutionRequest request, string sessionId)
    {
        var context = this.GetOrCreateContext(sessionId);
        ...
        try
        {
            ...
            {
                return this.ConvertAgentUpdate(agentUpdate, context);
            }

            // Handle Workflow events
            if (rawEvent is WorkflowEvent workflowEvent)
            {
                return this.ConvertWorkflowEvent(workflowEvent, context);
            }

            // Handle ChatMessage (fallback)
            if (rawEvent is ChatMessage chatMessage)
            {
                return this.ConvertChatMessage(chatMessage, context);
            }

            // Unknown event type
            this._logger.LogWarning("Unknown event type: {Type}", rawEvent.GetType().Name);
            return [];
        }
        catch (Exception ex)
        {
            this._logger.LogError(ex, "Error converting event");
            return [this.CreateErrorEvent(ex.Message, context)];
        }
    }
    ...
    private List<object> ConvertAgentUpdate(AgentRunResponseUpdate update, ConversionContext context)
    {
        ...
        foreach (var content in update.Contents)
        {
            try
            {
                ...
                {
                    TextContent text => this.MapTextContent(text, context),
                    FunctionCallContent funcCall => this.MapFunctionCallContent(funcCall, context),
                    FunctionResultContent funcResult => this.MapFunctionResultContent(funcResult, context),
                    UsageContent usage => this.MapUsageContent(usage, context),
                    ErrorContent error => this.MapErrorContent(error, context),
                    DataContent data => this.MapDataContent(data, context),
                    UriContent uri => this.MapUriContent(uri, context),
                    _ => this.MapUnknownContent(content, context)
                };
            ...
            {
                this._logger.LogWarning(ex, "Error mapping content type {Type}", content.GetType().Name);
                events.Add(this.CreateErrorEvent($"Content mapping failed: {ex.Message}", context));
            }
        }
    ...
    private List<object> MapTextContent(TextContent content, ConversionContext context)
    {
        ...
        var events = new List<object>
        {
            new ResponseTextDeltaEvent
            {
                ...
                Delta = text,
                SequenceNumber = this.NextSequence(context),
                Logprobs = []
            }
        };
    ...
    private List<object> MapFunctionCallContent(FunctionCallContent content, ConversionContext context)
    {
        ...
        if (!string.IsNullOrEmpty(content.CallId) && !string.IsNullOrEmpty(content.Name))
        {
            ...
            if (!context.ActiveFunctionCalls.ContainsKey(content.CallId))
            {
                ...
                events.Add(new
                {
                    ...
                    sequence_number = this.NextSequence(context)
                });
            }
        }
        ...
        if (content.Arguments != null)
        {
            ...
            var activeCall = this.GetActiveFunctionCall(content, context);
    ...
    public object ConvertWorkflowEvent(WorkflowEvent workflowEvent, string sessionId, int sequenceNumber)
    {
        ...
        return new ResponseWorkflowEventComplete
        {
    ...
    public object CreateChatCompletion(DevUIExecutionRequest request, string content, string? sessionId = null)
    {
    ...
    public object CreateErrorResponse(DevUIExecutionRequest request, string errorMessage, string? sessionId = null)
    {
        var context = this.GetOrCreateContext(sessionId ?? Guid.NewGuid().ToString());
        return this.CreateErrorEvent(errorMessage, context);
    }

    public object CreateStreamingChunk(DevUIExecutionRequest request, string content, bool isComplete = false, string? sessionId = null)
    {
        var context = this.GetOrCreateContext(sessionId ?? Guid.NewGuid().ToString());
