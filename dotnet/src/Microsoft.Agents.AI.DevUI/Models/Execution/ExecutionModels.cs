...
#pragma warning restore CA1024 // Use properties where appropriate
    ...
    {
        if (this.Conversation is string conversationStr)
        {
            return conversationStr;
        }
        if (this.Conversation is JsonElement jsonElement && jsonElement.ValueKind == JsonValueKind.Object && jsonElement.TryGetProperty("id", out var idProp))
        {
            return idProp.GetString();
        }
        if (this.Conversation is Dictionary<string, object> dict && dict.TryGetValue("id", out var id))
        {
            return id?.ToString();
        }
        return null;
    ...
    public string GetInputAsString()
    {
        ...
        if (this.Input.Value.ValueKind == JsonValueKind.String)
        {
            return this.Input.Value.GetString() ?? string.Empty;
        }

        // For array/object, return JSON representation
        return this.Input.Value.GetRawText();
    }

    /// <summary>
    /// Check if input is an array
    /// </summary>
    public bool IsInputArray() => this.Input?.ValueKind == JsonValueKind.Array;
    ...
    public string GetLastMessageContent()
    {
        ...
        if (this.Input.Value.ValueKind == JsonValueKind.Array)
        {
            var items = this.Input.Value.EnumerateArray();
    ...
    public ChatMessage[] ToChatMessages()
    {
        ...
        {
            return [new ChatMessage(ChatRole.User, inputStr)];
        }
        return [];
    }
}
